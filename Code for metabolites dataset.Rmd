...
metabolites_new_sig$Time <- as.factor(metabolites_new_sig$Time)
metabolites_new_sig$`Sample ID` <- as.factor(metabolites_new_sig$`Sample ID`)

# Example for one metabolite (replace 'Metabolite_X' with actual column name)
# model <- lmer(`209` ~ Time + (1 | `Sample ID`), data = metabolites_new_sig)
# summary(model)

results <- lapply(names(metabolites_new_sig)[5:ncol(metabolites_new_sig)], 
                  function(metabolite) {
                     formula_str <- paste0("`", metabolite, "` ~ Time + (1 | `Sample ID`)")
                      model <- lmer(as.formula(formula_str), data = metabolites_new_sig)
                      summary(model)
                      })
# example
formula_str <- paste0("`", "100003151", "` ~ Time + (1 | `Sample ID`)")
...
results_df <- lapply(names(metabolites_new_sig)[5:ncol(metabolites_new_sig)], function(metabolite) {
  
  # Construct formula
  formula_str <- paste0("`", metabolite, "` ~ Time + (1 | `Sample ID`)")
  
  # Fit model using lmerTest::lmer() to get df
  model <- lmerTest::lmer(as.formula(formula_str), data = metabolites_new_sig)
